...
// @require     http://localhost:8000/LinkedIn_Search_Fixed.user.js
// ==/UserScript==
...
function() {
	...
	const SELECTORS = {
		JOBS_LIST: 'div.jobs-search-results-list',
		JOBS_LIST_CONTAINER: '.scaffold-layout__list-container',
		ACTIVE_JOB: 'div.jobs-search-results-list__list-item--active',
		JOB_CARD_COMPANY_NAME: 'a.job-card-container__company-name',
		JOB_CARD_POST_TITLE: '.job-card-list__title',
		JOB_SEARCH_RESULTS_FEEDBACK_CLASS: '.jobs-list-feedback',

		DETAIL_POST_TITLE: '.t-24.t-bold.jobs-unified-top-card__job-title',
		DETAIL_COMPANY: '.jobs-unified-top-card__company-name',

		NEXT_PAGE: '.artdeco-pagination__indicator--number.active',
		PREV_PAGE: '.artdeco-pagination__indicator--number.active',
	...
	function isHidden(jobCardEl) {
		...
		return node.classList.contains(SELECTORS.JOB_SEARCH_RESULTS_FEEDBACK_CLASS) ||
	...
	function isSeacrhActive(){
		let sBox = document.querySelector(SELECTORS.SEARCH_FIELD);
	...
	function keyListener(e) {
		...
		callIfAssigned(KEY_HANDLER[e.key]);
	}
	...
	function handleHidePost() {
		...
		const detailPostTitle = document.querySelector(
		SELECTORS.DETAIL_POST_TITLE);
	...
	function handleHideCompany() {
		...
		const detailCompany = document.querySelector(SELECTORS.DETAIL_COMPANY);
	...
	function handleNextPage() {
		const activePage = document.querySelector(SELECTORS.NEXT_PAGE);
	...
	function handlePrevPage() {
		const activePage = document.querySelector(SELECTORS.PREV_PAGE);
	...
	function getJobsList() {
		return document.querySelectorAll(SELECTORS.JOBS_LIST)[0];
	}
	...
	function updateDisplay() {
		...
		const jobsList = jobsListDiv.querySelector(SELECTORS.JOBS_LIST_CONTAINER);
		for (let job = jobsList.firstElementChild;
		     job && job.nextSibling;
		     job = nextJobEl(job)) {
			try {
	...
	/** Get active job card */
	function getActive() {
		const active = document.querySelector(SELECTORS.ACTIVE_JOB);
	...
	function shiftWorkPage(direction) {
		...
		const pgNext = iterVisibleJobPage(active, direction);
	...
	function getCompanyNode(node) {
		return node.querySelector(SELECTORS.JOB_CARD_COMPANY_NAME);
	}

	function getPostNode(node) {
		return node.querySelector(SELECTORS.JOB_CARD_POST_TITLE);
	}
	...
	GM_addStyle(`.${SELECTORS.JOB_SEARCH_RESULTS_FEEDBACK_CLASS} { display: none }`);
	GM_addStyle('.hidden { display: none }');
	GM_addStyle('.read { opacity: 0.3 }');

	console.log('Adding mutation observer');

	function jobListObserver(mutationsList, observer) {
		// Use traditional 'for loops' for IE 11
		for (let mutation of mutationsList) {
			// const target = mutation.target;
			if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
				console.log('Update queued, ' + mutation.type);
				queueUpdate();
	...
	const observer = new MutationObserver(jobListObserver);
	...
	const $ = window.jQuery;
	waitForKeyElements(SELECTORS.JOBS_LIST, actionFunction);
