...
class HL {
            ...
            new DAO().getSettings().then((settings: Settings) => {
                new DAO().getHighlightingLog().then((highlightingLog: HighlightingLog) => {
                    new DAO().getGroups().then((groups: Array<Group>) => {
                        this.runHighlight(dictionary, settings, highlightingLog, groups, timeStart);
                    });
                });
            });
        });
    ...
    private static runHighlight(dictionary, settings, highlightingLog, groups, timeStart) {
